...
url_version_map = {}
CINDER_V1_WARNING = False


def cinderclient(context):
    version = get_cinder_client_version(context)
    url = url_version_map[version]
...
def get_cinder_client_version(context):
    ...
    if not url_version_map.get(url):
        ...
        for version in valid_versions:
            if version in components[1]:
                ...
                if not CINDER_V1_WARNING and version == '1':
                    ...
                    LOG.warn(msg)
                    CINDER_V1_WARNING = True

                url_version_map[version] = url
                return version
